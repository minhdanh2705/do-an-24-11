...
class Student {
    ...
    static async create(studentData) {
        // Lưu ý: DB mới không có ngaySinh, noiSinh. Dùng idDiemDon thay vì diemDon
        ...
        if (idDiemDon) {
            ...
            if (stopCheck.recordset.length === 0) throw new Error(`Điểm dừng ID ${idDiemDon} không tồn tại`);
        }
        ...
        if (idPhuHuynh) request.input('idPhuHuynh', sql.Int, parseInt(idPhuHuynh));
        ...
        if (idDiemDon) request.input('idDiemDon', sql.Int, parseInt(idDiemDon));
    ...
    static async update(id, studentData) {
        const pool = await poolPromise;
        const checkResult = await pool.request().input('id', sql.Int, id).query('SELECT idHocSinh FROM HOCSINH WHERE idHocSinh = @id');
        ...
        if (hoTen !== undefined) { request.input('hoTen', sql.NVarChar, hoTen); updates.push('hoTen = @hoTen'); }
        if (lop !== undefined) { request.input('lop', sql.NVarChar, lop); updates.push('lop = @lop'); }
        if (idPhuHuynh !== undefined) { request.input('idPhuHuynh', sql.Int, idPhuHuynh); updates.push('idPhuHuynh = @idPhuHuynh'); }
        if (idTuyen !== undefined) { request.input('idTuyen', sql.Int, idTuyen); updates.push('idTuyen = @idTuyen'); }
        if (idDiemDon !== undefined) { request.input('idDiemDon', sql.Int, idDiemDon); updates.push('idDiemDon = @idDiemDon'); }
        if (trangThai !== undefined) { request.input('trangThai', sql.Int, trangThai); updates.push('trangThai = @trangThai'); }
    ...
    // Lấy phụ huynh (Sửa lại logic JOIN trực tiếp)
    static async getLinkedParents(studentId) {
        const pool = await poolPromise;
        // Vì quan hệ 1-N (1 Học sinh có 1 idPhuHuynh), query trực tiếp qua khóa ngoại
        const result = await pool.request().input('studentId', sql.Int, studentId)
            .query(`
                ...
                WHERE H.idHocSinh = @studentId AND P.trangThai = 1
            `);
        return result.recordset;
    }
}
